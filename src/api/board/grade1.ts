...
async function insertBoard(req: Request, res: Response) {
    try {
        ...
        const rows = await db(
            "INSERT INTO Grade1 (title, body, user_id) values (?, ?, ?)",
            [title, body, user_id]
        );
        res.send({
            success: true,
...
async function searchBoard(req: Request, res: Response) {
    try {
      ...
      from Grade1 where title like ? or body like ? order by regdate desc`,
            ["%" + title + "%", "%" + title + "%"]
        );
...
async function readAllBoard(req: Request, res: Response) {
    try {
      ...
      (select count(board_id) from reply where Grade1.board_id=reply.board_id) as replyCount, 
      (select count(board_id) from scrap where Grade1.board_id=scrap.board_id) as ScrapCount 
      from Grade1 order by regdate desc`,
            []
        );
...
async function updateBoard(req: Request, res: Response) {
    try {
        ...
        const check = await db(
            "SELECT user_id FROM Grade1 WHERE board_id=? AND user_id=?",
        ...
        const rows = await db(
            "UPDATE Grade1 SET title=?, body=? WHERE board_id=?",
            [title, body, board_id]
        );
        res.json({
            success: true,
...
async function deleteBoard(req: Request, res: Response) {
    try {
        ...
        const rows = await db("DELETE FROM Grade1 WHERE board_id=?", [
            board_id,
        ]);
        res.json({
            success: true,
...
async function readScrapBoard(req: Request, res: Response) {
    try {
      ...
      (select count(board_id) from reply where Grade1.board_id=reply.board_id) as replyCount, 
      (select count(board_id) from scrap where Grade1.board_id=scrap.board_id) as ScrapCount 
      from scrap inner join Grade1 on Grade1.board_id=scrap.board_id where scrap.user_id=? order by regdate desc`,
            [userId]
        );
...
async function goodBoard(req: Request, res: Response) {
    try {
        ...
        if (check[0]) {
            await db("DELETE FROM boardgood WHERE good_id=?", [
                check[0].good_id,
            ]);
            res.json({
                success: true,
